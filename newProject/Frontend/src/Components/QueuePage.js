...
const QueuePage = () => {
  ...
  const patientName = localStorage.getItem("patientName"); // ✅ GET NAME

  useEffect(() => {

    // ✅ JOIN QUEUE (FINAL FIXED)
    const joinQueue = async () => {
      try {
        console.log("SENDING NAME:", patientName); // 🔍 DEBUG
        ...
        await fetch("https://medicare-full-project.onrender.com/api/queue/join", {
          ...
          body: JSON.stringify({
            email: userEmail,
            patientName: patientName // ✅ IMPORTANT FIX
          })
        });
